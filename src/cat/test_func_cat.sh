...
testing()
{
    ...
    ./alex_cat $t > test_alex_cat.log
    cat $t > test_sys_cat.log
    DIFF_RES="$(diff -s test_alex_cat.log test_sys_cat.log)"
    (( COUNTER++ ))
    if [ "$DIFF_RES" == "Files test_alex_cat.log and test_sys_cat.log are identical" ]
    then
      (( SUCCESS++ ))
    ...
    rm test_alex_cat.log test_sys_cat.log
}
